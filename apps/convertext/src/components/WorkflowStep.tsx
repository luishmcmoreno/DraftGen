...
const WorkflowStep: React.FC<WorkflowStepProps> = ({
  ...
  return (
    <div className="relative">
      {/* Timeline line */}
      {!isLastStep && <div className="absolute left-6 top-16 bottom-0 w-0.5 bg-border" />}
            ...
            <div className="flex items-center justify-between pt-4 border-t border-border">
              <div className="flex space-x-2">
                {step.status === 'completed' && step.output && (
                  <Button onClick={handleAddNewStep} size="sm" className="text-xs">
                ...
                {step.status === 'completed' && step.output && (
                  <Button
                    onClick={() =>
                      step.output &&
                      navigator.clipboard.writeText(step.output.result.converted_text)
                    }
                    variant="outline"
                    size="sm"
                    className="text-xs"
