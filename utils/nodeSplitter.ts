...
async function splitTextNode(
  ...
  while (low <= high) {
    ...
    for (let i = 0; i < rects.length; i++) {
      rangeHeight = Math.max(rangeHeight, rects[i].bottom - tempContainer.getBoundingClientRect().top);
    }
...
export async function splitNodeAtHeight(
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (testHeight <= availableHeight) {
      ...
      const currentHeight = fittingChildren.length > 0 
  ...
  if (config.minChildren && fittingChildren.length < config.minChildren && fittingChildren.length < children.length) {
